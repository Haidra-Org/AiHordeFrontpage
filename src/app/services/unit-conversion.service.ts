...
export class UnitConversionService {
   ...
   *
   * The `/v2/status/models` endpoint returns `performance` as pixelsteps/second for image models.
   * This converts to megapixelsteps/second (mps/s) for display.
   *
   * @param pixelstepsPerSecond - Performance rate from the model API (pixelsteps/s)
   * @returns SynthesizedUnit with mps/s as primary and standard images/s as technical
   */
  formatModelPerformanceImage(pixelstepsPerSecond: number): SynthesizedUnit {
    // Convert raw pixelsteps/s to megapixelsteps/s
    const megapixelstepsPerSecond = pixelstepsPerSecond / 1e6;
  ...
  // ============================================================================
  // WORKER-SPECIFIC UNIT FORMATTING (for /v2/workers endpoint)
  // ============================================================================

  /**
   * Formats worker performance for image workers.
   *
   * The worker API returns `performance` as megapixelsteps/second (already converted).
   *
  ...
  /**
   * Formats worker performance for text workers.
   *
   * The worker API returns `performance` as tokens/second.
   *
   * @param tokensPerSecond - Performance rate from the worker API (tokens/s)
   * @returns SynthesizedUnit with tokens/s as primary and pages/s as technical
   */
  formatWorkerPerformanceText(tokensPerSecond: number): SynthesizedUnit {
    // Convert to pages per second
    const pagesPerSecond = (tokensPerSecond * 0.75) / 500;
  ...
  /**
   * Formats total megapixelsteps generated by a worker.
   *
   * @param megapixelsteps - Total megapixelsteps generated
   * @returns SynthesizedUnit with megapixelsteps as primary and standard images as technical
   */
  formatWorkerMegapixelstepsGenerated(megapixelsteps: number): SynthesizedUnit {
    // Convert to standard images (1 standard image = 20 megapixelsteps)
    const standardImages = megapixelsteps / 20;
  ...
  /**
   * Formats total tokens generated by a worker.
   *
   * @param tokens - Total tokens generated
   * @returns SynthesizedUnit with tokens as primary and pages as technical
   */
  formatWorkerTokensGenerated(tokens: number): SynthesizedUnit {
    const pagesOfText = (tokens * 0.75) / 500;

    return {
      primary: this.formatWithSiPrefix(tokens, 'tokens', 1),
      technical: this.formatWithHumanPrefix(pagesOfText, 'pages of text', 1),
      rawValue: tokens,
      explanationKeys: [
        'pages_of_text.tooltip.line1',
